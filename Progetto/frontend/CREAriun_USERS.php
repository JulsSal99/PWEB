...
<html>
    ...
    <body>
        ...
        <div class="DIVbox NEWpage " style="text-align: left">
            <form method="GET" action="CREAriun_USERSruolo.php?">
            <p style="display: inline; margin-left: 20px;">ordina per: </p>
            <input class=BTNreg value="luogo" style="width:100px; margin-top: 10px; margin-bottom: 20px;" id="btnSubmitR" type="button" disabled="disabled">
            <input class=BTNreg type="submit" value="categoria" style="width:100px; margin-top: 10px; margin-bottom: 20px;" id="btnSubmitR" type="button">
        ...
        <!--<table class="registrazione"> -->
            ...
            if ($res != NULL){
                while($row = $res->fetch_assoc()) {
                    if ($row["Email"] != $mail){
                        if ($dip_prev == ""){ //primo dipartimento
                            echo '<details><summary>
                            <input type="checkbox" class="UsersDIPinput" id="'.$row["nome_Dipartimento"].'" value="" onclick="DIPchk('.$row["nome_Dipartimento"].'), Numero()" />'
                            .$row["nome_Dipartimento"].
                            '</summary><table>';
                        } else if ($row["nome_Dipartimento"] != $dip_prev){ //non è il primo dipartimento e è cambiato il dipartimento
                        ...
                        $dip_prev = $row["nome_Dipartimento"];
                    }
                }
        ...
        function DIPchk($dip){
            ...
            if ($res != NULL){
                while($row = $res->fetch_assoc()) {
                    ...
                    if ($row["Email"] != $mail){
                        echo 'if ($dip == '.$row["nome_Dipartimento"].'){';
                            echo '
                            if((document.getElementById("'.$row["nome_Dipartimento"].'")).checked){
